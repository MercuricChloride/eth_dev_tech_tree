...
export function getDataLinks(dataNodes: DataNode[]) {
  const filteredNodes = dataNodes.filter(dataNode => dataNode.dependencies !== undefined);
  return filteredNodes
    .map(dataNode => {
      if (dataNode.dependencies === undefined) return [];
      return dataNode.dependencies.map(dependency => {
        return {
          source: dataNode.id,
          target: dependency - 1,
        };
